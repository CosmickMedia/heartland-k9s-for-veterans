...
- a record whose mapped post is **in the trash** is left alone for the whole pass (`posts_stub` counts a skip and warns `Mapped post #id is in the trash on this site; left there`) — an editor trashing a page is an edit; restore it from the trash or run with *Overwrite conflicts* to create a fresh copy (plugin 1.3.2; before, a new post was created);
