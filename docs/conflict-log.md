...
Scope: every place where the Replit reference copy (`discovery/ref/reference-content.json`) and the live site (`discovery/live/content/*.json`, `discovery/discovery-result.json`) disagree, plus every content-driven layout deviation, every copy edit made while authoring `payload-src/` (reference pages in §§2–10, migrated live pages in §12 — the per-URL detail is in `docs/migration-map.md`), and the deliberate theme rendering deviations from the reference (§13, detail in `docs/visual-comparison.md`). Authored 2026-09-11 against live captures dated 2026-09-10; §§12–13 merged after the wave-3 review so this file is the single complete list.
...
| S13 | Reference values copy (no checkable fact) | "a veteran has already paid the price" (Home card 1, About legacy); "we forge partnerships that save lives" (Home mission); "Your service earned this" (Veterans hero); "It takes a community to save a life" (Get Involved hero) | Not on live | Kept as editable design copy under P3 (each is a values statement, not a factual claim). Editable in the page sections; no client decision required. | `ref-home.json`, `ref-about.json`, `ref-veterans.json`, `ref-get-involved.json` |
...
| R2 | 13 `hk9_partner` | Back the Pack FooGallery 2094 (live order) | Titles from alt text where present (Pinnacle Pet, Petland, Trademark Poodles, Veterans of Foreign Wars, Compton CPA Group P.C.), otherwise read from the legible logo artwork (Jack Henry & Associates, Joplin Outlaws, Knotty Reelz Outdoors, Union Station Deli) or the filename (Bennett, Alice CBD, Cup O Joe) — names to be confirmed by the client (unresolved U7). `OIP.jpg` is the "Petland Charities" logo that links to petlandjoplin.com on the Campaigns page → titled "Petland Charities" with that website; Pinnacle Pet gets the customer.pinnaclepet.net link from Campaigns. All other websites empty. All tagged `back-the-pack`; Pinnacle Pet, Petland Charities and Jack Henry & Associates additionally `campaign-sponsor` (they appear in the Campaigns thank-you blocks). The Campaigns-page Pinnacle logo variant (media 2926) is not a separate partner. |
| R3 | 5 `hk9_campaign` | Campaigns (2800) | Live order; bodies verbatim as blocks (empty spacer h1s dropped). "Paws" for Giving: the stale line "will be available by 2023!" removed (per discovery disposition) — stickers described as 4"×4", no cost. Sponsor relationships: Coloring Book → Pinnacle Pet; One of Us → Pinnacle Pet, Petland Charities, Jack Henry & Associates. CTA labels verbatim; all contact CTAs → `ref:page:contact`; "Go to the Book page" → `live:page:3627`. Status `active`, no dates/goals (none on live). One of Us has no campaign image on live → no featured image. |
| R4 | 1 `hk9_event` | Events (2483) | Start 2026-07-25 18:00 America/Chicago (gates), game 7 PM in body; end unknown → empty. Venue "Joe Becker Stadium", address "Joplin, MO" (no street address on live). Ticket info and SimpleTix registration verbatim. Status `scheduled`; the plugin's date logic classifies it as past, so the Events page shows it under past events and an empty "upcoming" state until new events are added (accepted; not a client decision). Older flyers attached to the Events page are media only (not fabricated as events). |
...
## 12. Copy edits on migrated live pages

Everything below is also recorded per URL in `docs/migration-map.md`; this section is the consolidated list. Policy: live text is kept verbatim except for obvious typos, stale facts and dead integrations; nothing is invented.

| # | Page(s) | Live | Payload | Reason |
|---|---|---|---|---|
| M1 | `/thank-you/` (2505) | Mailing address "PO Box 101, Saginaw, MO 64864" | "12651 Gateway Dr, Neosho, MO 64850" | Stale address; every other live page (Donate, Contact, footer) gives Gateway Dr. Heading "Click The Image to Print or Download the Medical History Form" → "Print or Download the Medical History Form" (the file is a download block, there is no image to click). |
| M2 | `/service-dogs-and-the-ada/` (1995) | "Frequently Asked Questions" button → `https://archive.ada.gov/` | `https://www.ada.gov/resources/service-animals-faqs/` | The archive root is not the FAQ; the DOJ FAQ page is. The four DOJ Q/A items were moved into the `faq` section (questions sentence-cased, "QUESTION:" prefix dropped, answers verbatim). |
| M3 | Typo fixes | "guidlines" (3093); "to the extend allowed by law" (Donate tax statement); "If you would like the join the pack" (2072); "spread the world about HK9" (3408); "Educate you patrons" (Ambassador campaign); "Human- Animal bond" (Chris Flemming bio); "and now work in the tech industry" (Cody Simmons bio); "alllll obedience training" (Marilyn & Travis team); "highlighted page in that includes" (Coloring Book Platinum tier) | "guidelines"; "extent"; "to join the pack"; "spread the word"; "your patrons"; "Human-Animal bond"; "works in the tech industry"; "all obedience training"; "Highlighted page that includes" | Unambiguous typos. Everything else in those sentences is verbatim. |
| M4 | `/heartland-gear/` (3408) | Two PayPal ordering paragraphs ("Thank you for helping HK9! To order, simply click the button below! Your donation will go through paypal…", "Not local to the 4 state area? Please add $5 for shipping…") | Removed | Orders now go through Links Ink ("BROWSE NOW" → linksinkllc.com kept as the CTA); the PayPal ordering flow no longer exists on the page. |
| M5 | `/campaigns/` → "Paws" for Giving record | "The stickers we have on order are 4"x4" and will be available by 2023!" | "The stickers are 4"x4"." | Stale date (see unresolved U10 for the campaign-status question). |
| M6 | `/campaigns/` → One of Us + Educational Coloring Book records | Sponsor-logo strips under the "Thank you for participating…" headings | Logos render from the `hk9_sponsors` relationship; no sentence under the headings | A previously authored line "We are grateful to the businesses participating in this campaign." was removed in the wave-3 fix — it was not live text. |
| M7 | `/the-hk9-coloring-book/` (3627) | Two tier groups, each introduced by the same "This educational coloring book will be featured…" sentence plus one group-specific sentence; Classroom tier ends "Sign up now!" | One `tiers.intro` = the shared sentence + both group sentences verbatim ("In THIS sponsorship, these books are YOURS for distribution!", "This is for dedicated PAGE sponsor levels that can be included in our coloring books."); "Sign up now!" kept; CTA band = live heading only | The theme renders one intro above all eight tiers. A merged paraphrase of the two sentences and an authored CTA line ("Ready to sponsor a tier or a classroom?…") were replaced in the wave-3 fix. Alt text authored for the cover and classroom images (media alts were the file names). |
| M8 | `/5-questions/`, `/thank-you/`, `/how-it-works-veteran-consideration/`, `/the-hk9-coloring-book/` | — | Hero band renders the page title only (`hero_band.text` and excerpt empty) | The hero text had repeated the opening paragraph (and, on 5 Questions, a shortened paraphrase of it), so each sentence showed twice. Meta descriptions fall back to the content. `/donate/` still carries the tax statement's second sentence as hero text/excerpt (comma added) — recorded in the map, left as is. |
| M9 | Decomposed pages (`/events/`, `/campaigns/`, `/meet-the-team/`, `/hk9-current-teams-in-training/`, `/donate/`) | No intro copy on live (or copy that belonged to one item) | Short UI copy in hero/CTA/option cards — e.g. Events CTA "…for our events." generalised from "…for this event.", Campaigns CTA paraphrasing the One of Us contact line, Donate option-card blurbs | Listing templates need a line of copy; each line is listed in the map with its live source or marked as UI copy. |
| M10 | Title case | "CAMPAIGNS", "The Hk9 Coloring Book" | "Campaigns", "The HK9 Coloring Book" | Page titles only; slugs unchanged. |
| M11 | **Kept verbatim on purpose (flagged)** | "HK9 does not provided funding…" (3093); "10 weeks and 16 months" vs "older than 14 months" (3084); the 2020 privacy-policy wording (contact-page form, Google ad cookies); "Item Retreival" in a registry task line; "Chris Flemming" (heading) vs "Chris Fleming" (bio) | Unchanged | Not typos that can be corrected without a factual decision — see unresolved U5, U6 and the privacy-policy note in the map. |
| M12 | Registry records (15 + master) | Handler phone numbers, one residential address, unclassifiable lines | Moved to admin-only `_hk9_review_notes`; emergency-contact and vet-contact lines stored verbatim; handler names dropped from titles | Registry purpose vs. household privacy — scope decision pending (unresolved U15). |

## 13. Theme rendering deviations from the reference (deliberate)

Measured and justified in `docs/visual-comparison.md` §§2, 6, 7; listed here so the conflict log is complete.

| # | Area | Reference | Theme | Reason |
|---|---|---|---|---|
| F1 | Home BarKode tile on mobile | The `w-full` tile inside an `items-center` flex column collapses to 0×0 below 1024 px — photo and caption disappear | Tile stays visible (358 px square at 390; section 1098 vs 631 px tall) | The collapse is a layout bug in the reference, not a design intent; the dedication photo is content. |
| F2 | Footer tagline colour | `text-secondary` #b82e45 on navy = 2.26:1 (fails WCAG AA) | Accessible tint `--hk9-secondary-on-primary` #dd7788 = 4.55:1, recomputed from the palette settings | Contrast compliance; hue/saturation kept. |
| F3 | Footer social links | None in the reference | Brand glyphs (facebook, instagram, youtube, linkedin — from the last lucide release that shipped them, ISC) as `aria-label`ed icon links; X renders as a text label | Settings-driven links the live site has (Facebook); lucide 1.44 has no brand icons. |
| F4 | Gallery captions (`/photos/`) | Core caption overlay is a scrollable region (axe "serious": unfocusable scroll container at 390) | Clamped to 4 lines, `overflow:hidden`; full text stays in the DOM and the lightbox | Accessibility; the reference has no gallery page. |
| F5 | Veterans hero stars pattern | Drawn with `currentColor` (page foreground) — near-invisible dark stars on navy | Same rendering, token named `--hk9-fg` | Matches the reference; noted because the token name looks like an oversight. |
| F6 | Mobile timeline order / sr-only headings / featured-story link / contact rows as `tel:`/`mailto:` | DOM order 1, 3, 2, 4; no section headings on stories list; plain text contact rows | Order 1…N on mobile; sr-only "Ways to get involved", "More Stories", "Teams in Training" headings; "Read the full story" link; linked contact rows | Accessibility and usability (inherited from wave 2). |
| F7 | CTA button rows on mobile | Single primary buttons stay inline; two-button rows stretch | Same, via the automatic `--multi` modifier | Matches the reference measurements. |
| F8 | `sizes="auto"` | Not applicable (Vite build) | Stripped from theme-rendered images that pass an explicit `sizes` | Prevents srcset re-selection during capture; block-content images keep core behaviour. |
| F9 | Placeholder testimonials / quote cards | Two fabricated quote cards on Stories, fabricated home testimonial | Not reproduced — only the verified Madison Stratton story; the list shows an empty state until more stories are published | P4 (nothing invented). |
