...
6. Check **Heartland → Settings → Forms**: confirm the recipient addresses (defaults: contact → info@heartlandk9s.org, application inquiries → director@heartlandk9s.org) and that the host can send mail (`wp_mail`); the Submissions screen shows whether each notification was sent.
   - **Gravity Forms (your licence)**: install and activate it — ideally before the import, so the end of the run creates the two Heartland forms ("Contact", "Initial Application Inquiry") automatically with the imported recipients; otherwise they are created the first time **Settings → Forms** is opened afterwards, or with the **Create the Heartland forms in Gravity Forms** button on that tab (CLI: `wp hk9 gravity provision`). The site then uses those forms (Default form provider: Gravity Forms), styled like the built-in ones, and contact / application submissions live under **Forms → Entries**; the notification recipients are copied from the addresses above into each form's *Admin notification* (edit them there from then on). Without Gravity Forms the built-in forms keep working.
...
4. Review the open decisions in `docs/unresolved.md` (office days, imagery, PayPal retirement, registry privacy scope, form recipients) and set them under **Heartland → Settings**; confirm the form recipient addresses (**Heartland → Settings → Forms**) and send one test message through the contact form.
   - **Gravity Forms**: install and activate your licensed copy. If it was active during the import, the *Next steps* box confirms that "Contact" and "Initial Application Inquiry" were created and are in use; if you install it later, open **Settings → Forms** (the forms are created on that first visit — or click **Create the Heartland forms in Gravity Forms**). If you opened Settings → Forms with Gravity Forms active *before* running the import, the forms were created with the site admin address as recipient; the import's last step replaces it with the imported recipients unless you have edited the notification in Gravity Forms since. From then on submissions are under **Forms → Entries** and recipients are edited in each form's *Notifications* (the Heartland recipient settings only seeded them). `wp hk9 gravity status` reports the state at any time.
...
5. Ensure outbound mail works (SMTP plugin of your choice) and test the contact and application forms once in production. With Gravity Forms in use, check the test entry under **Forms → Entries** and that the *Admin notification* reached the right inbox (Gravity Forms sends notifications in the background via WP-Cron — make sure cron runs on the host).
6. **Caching and page speed on Hostinger (LiteSpeed).** The theme is already tuned (WebP sizes, inline CSS, deferred fonts, one 3 kB script — `docs/performance.md`); the host only has to cache and not undo it.
   - **Purge after the import** and after every theme/plugin update: hPanel → Websites → *Cache* → Purge, or **LiteSpeed Cache → Toolbox → Purge All**. The old Avada pages are otherwise served from the cache until they expire. The importer changes content only; it never purges a host cache.
   - **LiteSpeed Cache plugin** (Hostinger's default; install/keep it): *Cache → Enable Cache: ON*, *Cache Logged-in Users: OFF*, *Cache Mobile: OFF* (the theme serves one HTML for every device — `<picture>`/`srcset` do the work — so a separate mobile cache only halves the hit rate), *Browser Cache: ON* with the default 1 year (this is what sends `Cache-Control: public, max-age=31557600` for `wp-content/uploads/…` and the theme's assets; WordPress never reuses a file name for a new upload and the theme's CSS/JS URLs carry a version, so a year is safe — only a re-run of `wp media regenerate` rewrites existing size files under the same names, which is why it is a one-off step, not a routine). *Purge → Purge All On Upgrade: ON*.
   - **Page Optimization: leave every switch OFF** — *CSS Minify/Combine* and *Load CSS Asynchronously* would act on Gravity Forms' stylesheets only (the theme's CSS is inline) and an async form stylesheet flashes an unstyled form; *JS Combine/Defer* is already done by the theme and by Gravity Forms; *Lazy Load Images: OFF* — the theme sets `loading` itself and the LCP hero must not be lazy; *Font Display Optimization* not needed (`swap` is in the `@font-face`). The one optional item: *Load JS Deferred → Delayed* for the Gravity Forms scripts raises the mobile Lighthouse score of `/contact/` and `/online-application/` (docs/performance.md §3) — test a submission on each form after enabling it.
   - **Image Optimization** (QUIC.cloud): optional. The theme's own sizes are WebP already; QUIC.cloud's *Image WebP Replacement* would additionally serve WebP for the core sizes of the 244 adopted photos (the same effect as running `wp media regenerate --yes` once after the import, which is the cheaper route: no external service, files stay on the server). Until one of those runs, an adopted image's `srcset` lists two formats — its old core sizes (JPEG, or PNG for the opaque PNG photos and flyers) next to the WebP sizes the import generated for the theme; that is valid and harmless (browsers pick by width as before), just worth knowing when you audit the Media Library — `docs/performance.md` §2/§4.
   - **HTTP/2 or HTTP/3 and gzip/brotli** are on by default on Hostinger — nothing to configure. Do not add Cloudflare "Rocket Loader" or similar script rewriting; it breaks Gravity Forms' inline initialisation.
   - **WebP on the host**: the theme asks the PHP image library for WebP sizes and falls back to JPEG by itself where it is missing; Hostinger's PHP 8.3 (GD with WebP, Imagick) supports it — confirm once with `wp eval 'var_dump( wp_image_editor_supports( [ "mime_type" => "image/webp" ] ) );'` or Site Health → Info → Media handling.
...
## F. Updating the site after launch (no hosting access needed)

Both packages check this GitHub repository for **published releases** (plugin-update-checker 5.7). A release is a tag `vX.Y.Z` on https://github.com/CosmickMedia/heartland-k9s-for-veterans/releases carrying `heartland-k9s-core.zip` and `heartland-k9s.zip` as assets; each package only accepts its own asset, so the plugin and the theme show up as separate updates.

1. Make the change in this repository (theme and plugin are versioned in lockstep).
2. Release: `npm run release -- 1.2.1 --notes="What changed"` — bumps `style.css`, `functions.php`, the plugin header/constant and `package.json`, prepends `CHANGELOG.md`, runs the builds, refreshes `build/` and `dist/`, commits `Release v1.2.1`, tags, pushes and creates the GitHub release with the ZIPs attached. (`--dry-run` stops before committing; `--no-push` commits/tags only.)
3. On the client's site (within 12 hours, or immediately via **Plugins → Heartland K9s Core → Check for updates** / **Dashboard → Updates**) WordPress lists *Heartland K9s Core 1.2.1* and *Heartland Canines for Veterans 1.2.1*; update them like any other plugin/theme. WP-CLI: `wp plugin update heartland-k9s-core && wp theme update heartland-k9s`.
